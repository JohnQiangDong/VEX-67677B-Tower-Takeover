...
void chassis_stop_hold(int i, int t)
{
...
void autostart(double left, double right, double speed)
{ //left and right must smaller than 200
  ...
  while (1)
  {
    if (Brain.timer(vex::timeUnits::msec) - t > 2000)
      break;
    ...
    if (errorvelocity < 0.7)
      errorvelocity = 0.7;
    if (instant_error > 0)
      right_vot *= errorvelocity;
    else
      left_vot *= errorvelocity;
...
void autobrake(double left_target, double right_target, double timelimit, double speed)
{
  ...
  while (1)
  {
    ...
    if (Brain.timer(vex::timeUnits::msec) - time > timelimit)
      break; //2000
    ...
    if (left_error > 0)
      left_vot = left_vot + left_constantV;
    if (left_error < 0)
      left_vot = left_vot - left_constantV;
    if (right_error > 0)
      right_vot = right_vot + right_constantV;
    if (right_error < 0)
      right_vot = right_vot - right_constantV;
    if (left_vot > maxV)
      left_vot = maxV;
    ...
    if (abs(left_error) > 1 || abs(right_error) > 1)
      t = Brain.timer(vex::timeUnits::msec);
    if (Brain.timer(vex::timeUnits::msec) - t >= 250)
      break;
  }
...
void autorun(double left_target, double right_target, double time, double speed)
{
  ...
  while (1)
  {
    if (Brain.timer(vex::timeUnits::msec) - t > time)
      break;
    left_error = left_target - left_1.rotation(vex::rotationUnits::deg);
    right_error = right_target - right_1.rotation(vex::rotationUnits::deg);
    if (fabs(left_error) <= distance || fabs(right_error) <= distance)
      break;
...
void automove(double left, double right, double time, double speed)
{
  ...
  start_left = left / 2;
  start_right = right / 2;
  if (start_left > start_distance)
    start_left = start_distance;
  if (start_left < -start_distance)
    start_left = -start_distance;
  if (start_right > start_distance)
    start_right = start_distance;
  if (start_right < -start_distance)
    start_right = -start_distance;
